...
def run_modules():
    ...
    if identity:
        #french touch
        if language == "fr" or language == "all":
        ...
        if pseudo:
            ...
            try:
                print("\033[36m Sherlock tool\033[0m")
                print(separator)
    ...
    if pseudo and not identity:
        ...
        try:
            print("\033[36m Sherlock tool\033[0m")
            print(separator)
            os.system("python3.9 tools/sherlock/sherlock/sherlock.py {} -o {}/{}.txt >/dev/null 2>&1".format(pseudo, dir_name, pseudo))
            with open("{}/{}.txt".format(dir_name, pseudo), "r") as result:
                for r in result.read().splitlines():
                    req = requests.get(r, verify=False, timeout=15)
                    if req.status_code not in [404, 403, 401, 500, 429]:
...
{}
    """.format("\033[32m{}\033[0m".format(pseudo) if pseudo else "N/A", "\033[32m{}\033[0m".format(firstname) if firstname else "N/A", "\033[32m{}\033[0m".format(middlename) if middlename else "", "\033[32m{}\033[0m".format(lastname) if lastname else "", "\033[32m{}\033[0m".format(mail) if mail else "N/A","\033[32m{}\033[0m".format(phone_n) if phone_n else "N/A", "\033[32m{}\033[0m".format(birth_year) if birth_year else "N/A", "\033[32m{}\033[0m".format(city) if city else "N/A", "\033[32m{}\033[0m".format([k for k in keyword]) if keyword else "N/A", "\033[32m{}\033[0m".format(picture) if picture else "N/A", separator))
...
if __name__ == '__main__':
    ...
    group.add_argument("-l", help="Language research: /all/en/fr/ ; \033[32mDefault: all\033[0m", dest='language', required=False, default="all")
    ...
    middlename = None #identity.split("_")[2] if identity and "-" in identity else None #TODO
