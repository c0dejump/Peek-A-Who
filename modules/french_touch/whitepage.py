    ...
    try:
        ...
        for rn in results_number:
            if firstname in rn.text and lastname in rn.text:
                number = rn.text
                print(" {}{}\n".format(info, number.replace("résultats pour","results for").replace("à","from").replace("\n","").replace("\t", " ").replace("  ", " ") if city else number.replace("résultats pour","results for").replace("\n","").replace("\t", " ").replace("  ", " ")))
                found = True
        if not found:
