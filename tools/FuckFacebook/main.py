...
socks.set_default_proxy(socks.SOCKS5, "127.0.0.1", 9050)  # 9050 Tor default PORT
...
def pass_the_captcha(URL_TOKEN, params, filters, max_results=None):
    ...
    main(req_captcha.url.split("=")[-1], params, filters, max_results)

def main(URL_TOKEN, params, filters, max_results=None):
    ...
    if "fill" in response.text:
        ...
        for _ in tqdm(range(10), desc="Processing before CAPTCHA", unit="step"):
            time.sleep(0.1)
        pass_the_captcha(URL_TOKEN, params, filters, max_results)
    else:
...
if __name__ == '__main__':
    ...
    main(URL_TOKEN, params, filters=None, max_results=args.max_results)
