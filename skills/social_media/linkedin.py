...
async def search_via_serp(
    ...
    def _fetch() -> list[dict]:
        ...
        def _consume(hits: list[dict], source_tag: str) -> None:
            for hit in hits:
                if len(results) >= max_results:
                    break
                real_url = hit.get("url", "")
        ...
        for i, query in enumerate(queries):
            ...
            if i > 0:
                time.sleep(1.0)
            try:
                res = web_search(query, num_results=max_results, region="fr-fr")
                _consume(res.get("results", []), res.get("engine_used", "search"))
            except Exception:
                pass
